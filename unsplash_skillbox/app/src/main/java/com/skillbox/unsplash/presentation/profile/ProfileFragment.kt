...
class ProfileFragment : Fragment(R.layout.fragment_profile) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        Timber.tag("LifecycleLog").d("${this.javaClass.simpleName} -> onViewCreated hash = ${this.hashCode()}")
        observeData()
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.profileInfoSharedFlow.collectLatest { profile ->
                viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                    profile.let {
                        when (it) {
                            is ResponseResultType.Success -> bindUserProfileData(it.content)
                            is ResponseResultType.Error -> showErrorMessage(it.throwable)
                            is ResponseResultType.Empty -> showPreloader()
                        }
                    }
                }
            }
